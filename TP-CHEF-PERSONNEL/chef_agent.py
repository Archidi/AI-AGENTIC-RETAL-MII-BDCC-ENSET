...
@tool
def searchRecipesRag(query: str) -> str:
...
@tool
def rememberPreference(preference: str, runtime: ToolRuntime) -> Command:
...
@tool
def getPreferences(runtime: ToolRuntime) -> str:
...
You have access to:
- searchRecipesRag: search your local recipe knowledge base
- searchWeb: search the internet for additional recipes and techniques
- rememberPreference: store user preferences, allergies, and dietary restrictions
- getPreferences: retrieve the user's stored preferences

When a user tells you their available ingredients:
1. First check their preferences with getPreferences
2. Search the recipe knowledge base with searchRecipesRag
...
When a user shares a preference, allergy, or dietary restriction, save it with rememberPreference.
...
chef_agent = create_agent(
    model=model,
    tools=[searchRecipesRag, searchWeb, rememberPreference, getPreferences],
